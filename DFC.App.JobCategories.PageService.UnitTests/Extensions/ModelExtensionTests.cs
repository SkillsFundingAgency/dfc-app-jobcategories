...
namespace DFC.App.JobCategories.Data.UnitTests.Extensions
{
    ...
    public class ModelExtensionTests
    {
        ...
        {
            // arrange
            var apiJobProfile = new JobProfileApiResponse { Description = "A Description", Title = "Comedian", Uri = new Uri("http://someuri/jobprofile/identifier"), Links = new List<Link> { new Link { Value = new KeyValuePair<string, Models.DynamicLink>("daytodaytask", new Models.DynamicLink { Href = new Uri("http://someuri/daytotasktask/identifier") }) } } };
